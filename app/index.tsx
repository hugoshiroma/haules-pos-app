...
const CLOSED_POSITION = SHEET_HEIGHT - FOOTER_HEIGHT;
...
export default function PosScreen() {
  ...
  // Resetar o BottomSheet quando o carrinho for limpo ou o primeiro item adicionado
  useEffect(() => {
    if (items.length === 0 || (items.length === 1 && items[0].quantity === 1)) {
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_, gestureState) => {
        ...
        if (newY >= OPEN_POSITION && newY <= CLOSED_POSITION) {
          panY.setValue(newY);
        }
      },
  ...
  const handleLoginSubmit = async () => {
    ...
    try {
      ...
      if (response) {
        ...
        setEmail('');
        setPassword('');
        showStatus('success', 'Login Realizado!', 'Bem-vindo ao Haules PoS.');
      }
    } finally {
  ...
  // Helper para ícones do modal de status
  ...
  if (showLogs) {
    return (
      <View style={[styles.container, { padding: 16, paddingTop: 60 }]}>
        <Text style={{ fontSize: 20, fontWeight: 'bold', marginBottom: 16 }}>Logs do Sistema</Text>
        <ScrollView style={{ flex: 1, backgroundColor: '#000', borderRadius: 8, padding: 8 }}>
  ...
  return (
    <View style={styles.container}>
        ...
        options={{ 
          ...
          headerRight: () => (
            <View style={{ flexDirection: 'row' }}>
              <TouchableOpacity onPress={() => router.push('/scanner')} style={{ marginRight: 15 }}><Text style={styles.headerButtonText}>Scanner</Text></TouchableOpacity>
              <TouchableOpacity onPress={activateTerminal} style={{ marginRight: 15 }}><Text style={styles.headerButtonText}>Ativar</Text></TouchableOpacity>
              <TouchableOpacity onPress={() => setShowLoginModal(true)} style={{ marginRight: 15 }}><Text style={[styles.headerButtonText, token && { color: '#4CAF50' }]}>{token ? 'Logado' : 'Login'}</Text></TouchableOpacity>
              <TouchableOpacity onPress={async () => { setLogContent(await getLogContent()); setShowLogs(true); }}><Text style={styles.headerButtonText}>Logs</Text></TouchableOpacity>
            </View>
          )
        }} 
      />
      ...
      {totalItems > 0 && (
        ...
        >
          <TouchableOpacity activeOpacity={1} onPress={() => toggleSheet(!isOpen)} style={styles.sheetHandleArea}>
            <View style={styles.handleBar} />
            <View style={styles.miniFooter}>
              <Text style={styles.miniFooterText}>{totalItems} itens • R$ {total.toFixed(2)}</Text>
              <Text style={styles.expandText}>{isOpen ? 'Arraste para fechar' : 'Arraste para ver itens'}</Text>
            </View>
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  sheetHandleArea: { alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderColor: '#f0f0f0' },
